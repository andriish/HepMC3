...
void GenEvent::add_tree(const std::vector<GenParticlePtr> &parts) {
    ...
    const std::shared_ptr<IntAttribute> existing_hc = attribute<IntAttribute>("cycles");
    ...
    while ( !sorting.empty() ) {
        HEPMC3_DEBUG_CODE_BLOCK(
            ...
            const GenVertexPtr &v = sorting.front();

        bool added = false;

        // Add all mothers to the front of the list
        for (const auto& p: v->particles_in() ) {
            const GenVertexPtr v2 = p->production_vertex();

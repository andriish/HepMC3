...
bool pair_GenVertexPtr_int_greater::operator()(const std::pair<ConstGenVertexPtr, int>& lx, const std::pair<ConstGenVertexPtr, int>& rx) const
{
    ...
    if (lx.first->particles_in_size() != rx.first->particles_in_size()) return (lx.first->particles_in_size() < rx.first->particles_in_size());
    if (lx.first->particles_out_size() != rx.first->particles_out_size()) return (lx.first->particles_out_size() < rx.first->particles_out_size());
    /* The code below is usefull mainly for debug. Assures strong ordering.*/
    std::vector<int> lx_id_in;
    lx_id_in.reserve(lx.first->particles_in_size());
    std::vector<int> rx_id_in;
    rx_id_in.reserve(rx.first->particles_in_size());
    ...
    lx_id_out.reserve(lx.first->particles_out_size());
    std::vector<int> rx_id_out;
    rx_id_out.reserve(rx.first->particles_out_size());

...
public:
    ...
    Writer(const std::string &filename, const LHEF::HEPRUP &heprup);
    ~Writer();
    ...
    void writeEvent();
    void writeEvent(const LHEF::HEPEUP &hepe);
...
bool Reader::readEvent(LHEF::HEPEUP &hepe) { if (!readEvent()) return false; hepe = hepeup; return true; }
...
bool Reader::failed() const { return m_failed; }

void Reader::close() { m_events_ds.reset(); m_particles_ds.reset(); m_file.reset(); }

Writer::Writer(const std::string &filename)
    : m_filename(filename) {}

Writer::Writer(const std::string &filename, const LHEF::HEPRUP &heprup_in)
    : heprup(heprup_in), m_filename(filename) {}

Writer::~Writer() = default;

void Writer::init() { initFile(); }

void Writer::initFile() {
    if (m_initialized) return;
    m_file.reset(new HighFive::File(m_filename, HighFive::File::Overwrite));
    std::vector<int> version_no = {2, 0, 1};
    auto version_ds = m_file->createDataSet<int>("version", HighFive::DataSpace::From(version_no));
    version_ds.write(version_no);
    int beamA = heprup.IDBMUP.first, beamB = heprup.IDBMUP.second;
    double energyA = heprup.EBMUP.first, energyB = heprup.EBMUP.second;
    int pdfG1 = heprup.PDFGUP.first, pdfG2 = heprup.PDFGUP.second;
    int pdfS1 = heprup.PDFSUP.first, pdfS2 = heprup.PDFSUP.second, weight_strat = heprup.IDWTUP;
    std::vector<ProcData> procs;
    for (int i = 0; i < heprup.NPRUP; ++i) {
        ProcData p;
        p.procId = i < static_cast<int>(heprup.LPRUP.size()) ? heprup.LPRUP[i] : i + 1;
        p.npLO = -1; p.npNLO = -1;
        p.xSection = i < static_cast<int>(heprup.XSECUP.size()) ? heprup.XSECUP[i] : 0.0;
        p.error = i < static_cast<int>(heprup.XERRUP.size()) ? heprup.XERRUP[i] : 0.0;
        p.unitWeight = i < static_cast<int>(heprup.XMAXUP.size()) ? heprup.XMAXUP[i] : 1.0;
        procs.push_back(p);
    }
    if (procs.empty()) procs.push_back({1, -1, -1, 0.0, 0.0, 1.0});
    std::vector<std::string> weight_names;
    for (std::size_t i = 0; i < heprup.weightinfo.size(); ++i) {
        std::string wname = heprup.weightNameHepMC(i);
        weight_names.push_back(wname.empty() ? heprup.weightinfo[i].name : wname);
    }
    if (weight_names.empty()) weight_names = {"NOMINAL"};
    m_nweights = weight_names.size();
    std::vector<double> idata = {static_cast<double>(beamA), static_cast<double>(beamB), energyA, energyB,
        static_cast<double>(pdfG1), static_cast<double>(pdfG2), static_cast<double>(pdfS1), static_cast<double>(pdfS2),
        static_cast<double>(weight_strat), static_cast<double>(procs.size())};
    auto init_ds = m_file->createDataSet<double>("init", HighFive::DataSpace::From(idata));
    init_ds.write(idata);
    std::vector<std::string> inames = {"beamA", "beamB", "energyA", "energyB", "PDFgroupA", "PDFgroupB", "PDFsetA", "PDFsetB", "weightingStrategy", "numProcesses"};
    init_ds.createAttribute<std::string>("properties", HighFive::DataSpace::From(inames)).write(inames);
    std::vector<std::vector<double>> pdata;
    ...
    auto proc_ds = m_file->createDataSet<double>("procInfo", HighFive::DataSpace::From(pdata));
    proc_ds.write(pdata);
    std::vector<std::string> pnames = {"procId", "npLO", "npNLO", "xSection", "error", "unitWeight"};
    proc_ds.createAttribute<std::string>("properties", HighFive::DataSpace::From(pnames)).write(pnames);
    std::vector<std::string> enames = {"pid", "nparticles", "start", "trials", "scale", "fscale", "rscale", "aqed", "aqcd"};
    enames.insert(enames.end(), weight_names.begin(), weight_names.end());
    HighFive::DataSpace space_events({0, 9 + m_nweights}, {HighFive::DataSpace::UNLIMITED, 9 + m_nweights});
    HighFive::DataSetCreateProps props_events;
    props_events.add(HighFive::Chunking(std::vector<hsize_t>{1024, 9 + m_nweights}));
    m_events_ds.reset(new HighFive::DataSet(m_file->createDataSet<double>("events", space_events, props_events)));
    m_events_ds->createAttribute<std::string>("events", HighFive::DataSpace::From(enames)).write(enames);
    std::vector<std::string> part_names = {"id", "status", "mother1", "mother2", "color1", "color2", "px", "py", "pz", "e", "m", "lifetime", "spin"};
    HighFive::DataSpace space_parts({0, 13}, {HighFive::DataSpace::UNLIMITED, 13});
    HighFive::DataSetCreateProps props_parts;
    props_parts.add(HighFive::Chunking(std::vector<hsize_t>{10240, 13}));
    m_particles_ds.reset(new HighFive::DataSet(m_file->createDataSet<double>("particles", space_parts, props_parts)));
    m_particles_ds->createAttribute<std::string>("properties", HighFive::DataSpace::From(part_names)).write(part_names);
    m_initialized = true;
}
...
void Writer::writeEvent() {
    ...
    for (int i = 0; i < hepeup.NUP; ++i) {
        double px = hepeup.PUP[i].size() > 0 ? hepeup.PUP[i][0] : 0.0;
        double py = hepeup.PUP[i].size() > 1 ? hepeup.PUP[i][1] : 0.0;
        double pz = hepeup.PUP[i].size() > 2 ? hepeup.PUP[i][2] : 0.0;
        double e = hepeup.PUP[i].size() > 3 ? hepeup.PUP[i][3] : 0.0;
        double m = hepeup.PUP[i].size() > 4 ? hepeup.PUP[i][4] : 0.0;
        double vtim = static_cast<size_t>(i) < hepeup.VTIMUP.size() ? hepeup.VTIMUP[i] : 0.0;
        double spin = static_cast<size_t>(i) < hepeup.SPINUP.size() ? hepeup.SPINUP[i] : 0.0;
        prows.push_back({static_cast<double>(hepeup.IDUP[i]), static_cast<double>(hepeup.ISTUP[i]), static_cast<double>(hepeup.MOTHUP[i].first), static_cast<double>(hepeup.MOTHUP[i].second), static_cast<double>(hepeup.ICOLUP[i].first), static_cast<double>(hepeup.ICOLUP[i].second), px, py, pz, e, m, vtim, spin});
    }
    std::vector<double> weights;
    if (!hepeup.weights.empty()) for (const auto &w : hepeup.weights) weights.push_back(w.first);
    else weights.push_back(hepeup.XWGTUP);
    if (weights.size() < m_nweights) weights.resize(m_nweights, 0.0);
    else if (weights.size() > m_nweights) weights.resize(m_nweights);
    ...
    ++m_event_offset;
    if (!prows.empty()) {
        m_particles_ds->resize({m_particles_offset + prows.size(), 13});
        m_particles_ds->select({static_cast<size_t>(m_particles_offset), 0}, {prows.size(), 13}).write(prows);
        m_particles_offset += prows.size();
    }
}

void Writer::writeEvent(const LHEF::HEPEUP &hepe) { hepeup = hepe; writeEvent(); }

bool Writer::failed() const { return m_failed; }

void Writer::close() { m_events_ds.reset(); m_particles_ds.reset(); m_file.reset(); }
